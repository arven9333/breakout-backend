...
from enums.map import ModelEnum
...
class ActionsHandleService:
    ...
    icon_metric_figure_service: FigureService
    ...
    async def handle_actions(self, actions: list[ActionScheme]):
        for action in actions:
            ...
            if action_model == ModelEnum.icon_metric_level:
                service = self.icon_level_service

            elif action_model == ModelEnum.icon_metric_figure:
                service = self.icon_metric_figure_service

            if service is not None:
                await service.match_action(action_type, **data)
